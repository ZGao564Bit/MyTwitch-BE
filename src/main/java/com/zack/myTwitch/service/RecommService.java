...
import com.zack.myTwitch.entity.response.Game;
...
public class RecommService {
    ...
    private List<Item> recommendByTopGames(ItemType type, List<Game> topGames) throws RecommException {
        ...
        for (Game game : topGames) {
            ...
            try {
                ...
                throw new RecommException("Failed to get recommendation result");
            }
            for (Item item : items) {
    ...
    private List<Item> recommendByFavoriteHistory(
            Set<String> favoritedItemIds, List<String> favoritedGameIds, ItemType type) throws RecommException {
        // Count the favorite game IDs from the database for the given user. E.g.
        // if the favorited game ID list is ["1234", "2345", "2345", "3456"], the returned Map is {"1234": 1, "2345": 2, "3456": 1}
        Map<String, Long> favoriteGameIdByCount = new HashMap<>();
        for(String gameId : favoritedGameIds) {
            favoriteGameIdByCount.put(
                    gameId,
                    favoriteGameIdByCount.getOrDefault(gameId, 0L) + 1
            );
        }
        ...
        if (sortedFavoriteGameIdListByCount.size() > DEFAULT_GAME_LIMIT) {
            sortedFavoriteGameIdListByCount = sortedFavoriteGameIdListByCount.subList(
        ...
        for (Map.Entry<String, Long> favoriteGame : sortedFavoriteGameIdListByCount) {
            List<Item> items;
            try {
                items = gameService.searchByType(
                        favoriteGame.getKey(),
                        type,
    ...
    public Map<String, List<Item>> recommendItemsByUser(String userId) throws RecommException {
        ...
        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) {
            if (entry.getValue().size() == 0) {
                ...
                try {
                    ...
                    throw new RecommException("Failed to get game data for recommendation");
                }
    ...
    public Map<String, List<Item>> recommendItemsByDefault() throws RecommException {
        Map<String, List<Item>> recommendedItemMap = new HashMap<>();
        List<Game> topGames;
        try {
            topGames = gameService.topGames(DEFAULT_GAME_LIMIT);
